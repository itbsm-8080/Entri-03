...
var
  ...
  i:integer;
begin
//  s:='call insertlistkas ('+quot('11.002')+','+QuotD(startdate.DateTime)+','+quotd(enddate.DateTime)+')';
//  SQLQuery2.SQLConnection := frmMenu.conn;
//  SQLQuery2.SQL.Clear;
//  SQLQuery2.SQL.Add(s);
//  SQLQuery2.ExecSQL;
...
//                  + ' union'
//                  + ' select jurd_jur_no Nomor,date_format(tanggal,"%Y-%m-%d") Tanggal,'
//                  + ' jurd_keterangan  Keterangan,rek_nama Rekening,jurd_kredit Debet,jurd_debet Kredit,'
//                  + ' 0 Saldo,jurd_cc_kode Memo from tjurnalitem b inner join'
//                  + ' ('
//                  + ' select jurd_jur_no nomor,jur_tanggal tanggal,jur_tipetransaksi,jur_keterangan'
...
//                  + ' on b.jurd_jur_no=a.nomor'
//                  + ' inner join trekening on rek_kode=jurd_rek_kode'
//                  + ' left join tbayarcus_hdr on byc_nomor=jurd_jur_no left join tcustomer on byc_cus_kode=cus_kode'
//                  + ' where jurd_rek_kode <> '+ quot(VarToStr(cxlookupRekeningcash.editvalue)) ;
//
 Self.SQLMaster :='select "" Nomor,'+quot(FormatDateTime('yyyy-mm-dd',startdate.datetime)) +' Tanggal,"Saldo Awal" Keterangan,"" Rekening,0 Debet, 0 Kredit, ifnull(sum(jurd_debet-jurd_kredit),0) Saldo,"" Memo '
                  + ' from tjurnalitem inner join tjurnal'
...
begin
  ...
  begin
    ...
    S := 'select rek_nama as Rekening, rek_kode Kode '
        +' from trekening where rek_nama not like '+quotedstr('%Laba%');
...
begin
  inherited;
  if pos(UpperCase('Pembayaran Lain'),UpperCase(CDSMaster.FieldByname('Keterangan').AsString)) > 0  then
  begin
